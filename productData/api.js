var vegetableArr = [
    {
        image_adress:"MasaiKnights-Bigbasket\product data\images\1203441_1-fresho-onion-1-kg-new-potato-1-kg-tomato-hybrid-1-kg.webp",
        name : "Onion 1 kg + New Potato 1 kg + Tomato - Hybrid 1 kg",
        Quantity : "Combo 3 Items",
        price : "Rs 105.40",
        strikedOdprice : "Rs 167.60"

    },
    {
        image_adress:"MasaiKnights-Bigbasket\product data\images\10000047_21-fresho-beetroot.webp",
        name : "Beetroot",
        Quantity : "1 Kg",
        price : "Rs 84",
        strikedOdprice : "Rs 105"

    },
    
    {
        image_adress:"MasaiKnights-Bigbasket\product data\images\10000050_20-fresho-bitter-gourd.webp",
        name : "Bitter Gourd",
        Quantity : "1 Kg",
        price : "Rs 64",
        strikedOdprice : "Rs 89"

    },
    {
        image_adress:"MasaiKnights-Bigbasket\product data\images\10000091_18-fresho-coccinia.webp",
        name : "Coccinia",
        Quantity : "500 g",
        price : "Rs 54",
        strikedOdprice : "Rs 68"

    },
    {
        image_adress:"MasaiKnights-Bigbasket\product data\images\10000143_14-fresho-ladies-finger.webp",
        name : "Ladies Finger",
        Quantity : "250 g",
        price : "Rs 18",
        strikedOdprice : "Rs 40"

    },
    {
        image_adress:"MasaiKnights-Bigbasket\product data\images\10000150_19-fresho-onion.webp",
        name : "Onion",
        Quantity : "5 kg",
        price : "Rs 101",
        strikedOdprice : "Rs 181.25"

    },
    {
        image_adress:"MasaiKnights-Bigbasket\product data\images\10000162_16-fresho-pumpkin-green.webp",
        name : "Pumpkin - Green",
        Quantity : "1 kg",
        price : "Rs 37",
        strikedOdprice : "Rs 54"

    },
    {
        image_adress:"MasaiKnights-Bigbasket\product data\images\10000200_17-fresho-tomato-hybrid.webp",
        name : "Tomato Hybrid/Tameta",
        Quantity : "1 kg",
        price : "Rs 41",
        strikedOdprice : "Rs 64"

    },
    {
        image_adress:"MasaiKnights-Bigbasket\product data\images\10000273_16-fresho-mushrooms-button.webp",
        name : "Mushrooms - Button",
        Quantity : "1 pack(200g)",
        price : "Rs 44",
        strikedOdprice : "Rs 55"

    },
    {
        image_adress:"MasaiKnights-Bigbasket\product data\images\10000326_14-fresho-coriander-leaves.webp",
        name : "Coriander Leaves",
        Quantity : "1 Kg",
        price : "Rs 493",
        strikedOdprice : "Rs 616"

    },
    {
        image_adress:"MasaiKnights-Bigbasket\product data\images\10000366_10-fresho-parwal.webp",
        name : "Parwal",
        Quantity : "500 g",
        price : "Rs 61",
        strikedOdprice : "Rs 76"

    },
    {
        image_adress:"MasaiKnights-Bigbasket\product data\images\10000586_10-fresho-brinjal-nagpur.webp",
        name : "Brinjal - Nagpur",
        Quantity : "1 Kg",
        price : "Rs 73",
        strikedOdprice : "Rs 91"

    },
    {
        image_adress:"MasaiKnights-Bigbasket\product data\images\20000745_5-fresho-bottle-gourd.webp",
        name : "Bottle Gourd",
        Quantity : "1 pc (500 g - 800 g)",
        price : "Rs 40",
        strikedOdprice : "Rs 60"

    },
    {
        image_adress:"MasaiKnights-Bigbasket\product data\images\30000583_4-fresho-sponge-gourd.webp",
        name : "Sponge Gourd",
        Quantity : "1 Kg",
        price : "Rs 96",
        strikedOdprice : "Rs 120"

    },
    {
        image_adress:"MasaiKnights-Bigbasket\product data\images\40162469_6-fresho-potato-new-crop.webp",
        name : "Beetroot/Beet",
        Quantity : "1 Kg",
        price : "Rs 84",
        strikedOdprice : "Rs 105"

    },
    {
        image_adress:"MasaiKnights-Bigbasket\product data\images\40200233_1-fresho-cabbage.webp",
        name : "Cabbage",
        Quantity : "1 pc(350 g - 500 g",
        price : "Rs 55",
        strikedOdprice : "Rs 68"

    }
]

var foodgrainsArr = [
    {
        image_adress:"MasaiKnights-Bigbasket\product data\images\40127505_7-aashirvaad-shudh-chakki-atta.webp",
        brand : "Aashirvad",
        name : "Shudh Chakki Atta - Rich In Protein & Dietary Fibre",
        Quantity : "10 Kg",
        price : "Rs 367",
        strikedOdprice : "Rs 426"

    },
    {
        image_adress:"MasaiKnights-Bigbasket\product data\images\30003360_7-dhara-oil-mustard-kachi-ghani.webp",
        brand : "Dhara",
        name : "Oil - Mustard (Kachi Ghani)",
        Quantity : "1 L Pouch",
        price : "Rs 167",
        strikedOdprice : "Rs 195"

    },
    {
        image_adress:"MasaiKnights-Bigbasket\product data\images\40019396_10-bb-popular-sugar.webp",
        brand : "BB Popular",
        name : "Sugar/Chini",
        Quantity : "1 Kg",
        price : "Rs 45.60",
        strikedOdprice : "Rs 55"

    },
    {
        image_adress:"MasaiKnights-Bigbasket\product data\images\10000407_12-bb-royal-refined-sugar-sulphurless.webp",
        brand : "BB Royal",
        name : "Refined Sugar (Sulphurless)/Chini",
        Quantity : "1 Kg",
        price : "Rs 52",
        strikedOdprice : "Rs 60"

    },
    {
        image_adress:"MasaiKnights-Bigbasket\product data\images\40026603_9-bb-royal-cuminjeera-whole.webp",
        brand : "BB Royal",
        name : "Cumin/Jeera - Whole",
        Quantity : "500 g pouch",
        price : "Rs 192",
        strikedOdprice : "Rs 300"

    },
    {
        image_adress:"MasaiKnights-Bigbasket\product data\images\10000428_17-bb-popular-toorarhar-dal.webp",
        brand : "BB Popular",
        name : "Toor/Arhar Dal",
        Quantity : "1 Kg pouch",
        price : "Rs 123.50",
        strikedOdprice : "Rs 160"

    },
    {
        image_adress:"MasaiKnights-Bigbasket\product data\images\40051744_5-fortune-fortune-premium-kachi-ghani-pure-mustard-oil.webp",
        brand : "Fortune",
        name : "Fortune Premium Kachi Ghani Pure Mustard Oil",
        Quantity : "1 L pouch",
        price : "Rs 157",
        strikedOdprice : "Rs 195"

    },
    {
        image_adress:"MasaiKnights-Bigbasket\product data\images\241600_5-tata-salt-iodized.webp",
        brand : "Tata Salt",
        name : "Iodized",
        Quantity : "1 kg Pouch",
        price : "Rs 22",
        strikedOdprice : "Rs 25"

    },
    {
        image_adress:"MasaiKnights-Bigbasket\product data\images\40113566_1-shree-neelkamal-sattu.webp",
        brand : "Shree Neelkamal",
        name : "Sattu",
        Quantity : "500 g",
        price : "Rs 74",
        strikedOdprice : "Rs 78"

    },
    {
        image_adress:"MasaiKnights-Bigbasket\product data\images\40099239_9-bb-royal-chakki-fresh-wheat-atta-fortified.webp",
        brand : "BB Royal",
        name : "Wheat Atta Chakki Fresh",
        Quantity : "10 Kg",
        price : "Rs 329",
        strikedOdprice : "Rs 425"

    },
    {
        image_adress:"MasaiKnights-Bigbasket\product data\images\1220744_1-aashirvaad-shudh-chakki-atta-10-kg-iodised-salt-1-kg.webp",
        brand : "Aashirvaad",
        name : "Shudh Chakki Atta 10 Kg + Iodised Salt 1 kg",
        Quantity : "Combo 2 Items",
        price : "Rs 381.40",
        strikedOdprice : "Rs 450"

    },
    {
        image_adress:"MasaiKnights-Bigbasket\product data\images\1220745_1-aashirvaad-shudh-chakki-atta-5-kg-iodised-salt-1-kg.webp",
        brand : "Aashirvaad",
        name : "Shudh Chakki Atta 5 kg + Iodised Salt 1 kg",
        Quantity : "Combo 2 Items",
        price : "Rs 212.40",
        strikedOdprice : "Rs 252"

    },
    {
        image_adress:"MasaiKnights-Bigbasket\product data\images\40044729_7-bb-royal-maida.webp",
        brand : "BB Royal",
        name : "Maida",
        Quantity : "2 Kg pouch",
        price : "Rs 78",
        strikedOdprice : "Rs 120"

    },
    {
        image_adress:"MasaiKnights-Bigbasket\product data\images\147491_9-saffola-gold-refined-cooking-oil-blended-rice-bran-sunflower-oil-helps-keeps-heart-healthy.webp",
        brand : "Saffola",
        name : "Gold Refined Cooking oil | Blended Rice Bran & Sunflower oil | Helps Keeps Heart Healthy",
        Quantity : "1 L pouch",
        price : "Rs 215",
        strikedOdprice : "Rs 250"

    },
    {
        image_adress:"MasaiKnights-Bigbasket\product data\images\40059444_2-engine-kacchi-ghani-mustard-oil.webp",
        brand : "Engine",
        name : "Kacchi Ghani Mustard Oil",
        Quantity : "1 L pouch",
        price : "Rs 190",
        strikedOdprice : "Rs 235"

    },
    {
        image_adress:"MasaiKnights-Bigbasket\product data\images\10000422_12-bb-royal-sooji-ordinarybombay-rava.webp",
        brand : "BB Royal",
        name : "Sooji Ordinary/Bombay Rava",
        Quantity : "500 g pouch",
        price : "Rs 23",
        strikedOdprice : "Rs 30"

    },

]
var dairyArr = [
    {
        image_adress:"https://www.bigbasket.com/media/uploads/p/s/1202684_3-amul-taaza-homogenised-toned-milk.jpg",
        brand : "Amul Taaza",
        name : "Homogenised Toned Milk",
        Quantity : "6X1 Multipack",
        price : "399.84",
        strikedOdprice : "408"

    },
    {
        image_adress:"https://www.bigbasket.com/media/uploads/p/s/104864_8-amul-butter-pasteurised.jpg",
        brand : "Amul",
        name : "Pasteurised Butter",
        Quantity : "500 g Carton",
        price : "247",
        strikedOdprice : "255"

    },
    {
        image_adress:"https://www.bigbasket.com/media/uploads/p/s/1207078_2-amul-masti-buttermilk-spice.jpg",
        brand : "Amul",
        name : "Masti Buttermilk - Spice",
        Quantity : "2x1 Multipack",
        price : "94.08",
        strikedOdprice : "100"

    },
    {
        image_adress:"https://www.bigbasket.com/media/uploads/p/s/1202761_3-amul-cheese-slices.jpg",
        brand : "Amul",
        name : "Malai Paneer",
        Quantity : "200 g Pouch",
        price : "82",
        strikedOdprice : "89"

    },
    {
        image_adress:"https://www.bigbasket.com/media/uploads/p/s/40045943_1-amul-butter-pasteurised.jpg",
        brand : "Amul",
        name : "Cheese Slices - Rich In Protein, Wholesome, No Added Sugar",
        Quantity : "2x200 g Multipack",
        price : "254.80",
        strikedOdprice : "260"

    },
    {
        image_adress:"https://www.bigbasket.com/media/uploads/p/s/1215454_1-britannia-bake-rusk-toast.jpg",
        brand : "Amul",
        name : "Pasteurised Butter",
        Quantity : "200 g Tub",
        price : "106",
        strikedOdprice : "116"

    },
    {
        image_adress:"https://www.bigbasket.com/media/uploads/p/s/1215454_1-britannia-bake-rusk-toast.jpg",
        brand : "Britannia",
        name : "Bake Rusk Toast",
        Quantity : "200 g Tub",
        price : "198",
        strikedOdprice : "219"

    },
    {
        image_adress:"https://www.bigbasket.com/media/uploads/p/s/104707_8-amul-homogenised-standardised-milk.jpg",
        brand : "Amul",
        name : "Gold Homogenised Standardised Milk",
        Quantity : "1 L Carton",
        price : "74",
        strikedOdprice : "80"

    },
    {
        image_adress:"https://www.bigbasket.com/media/uploads/p/s/40195180_2-moreish-brown-bread-whole-wheat-flour-rich-in-fibre-protein.jpg",
        brand : "Moreish",
        name : "Brown Bread - Whole Wheat Flour, Rich In Fibre & Protein`",
        Quantity : "400 g",
        price : "38",
        strikedOdprice : "40"

    },
    {
        image_adress:"https://www.bigbasket.com/media/uploads/p/s/279585_8-amul-malai-paneer-block.jpg",
        brand : "Amul",
        name : "Malai Paneer - Block",
        Quantity : "1 kg Carton",
        price : "378",
        strikedOdprice : "400"

    },
    {
        image_adress:"https://www.bigbasket.com/media/uploads/p/s/104615_9-amul-processed-cheese-cubes.jpg",
        brand : "Amul",
        name : "Processed Cheese Chiplets Cubes",
        Quantity : "200 g (8 Cubes)",
        price : "122",
        strikedOdprice : "128"

    },
    {
        image_adress:"https://www.bigbasket.com/media/uploads/p/s/40058175_2-amul-amulya-dairy-whitener.jpg",
        brand : "Amul",
        name : "Amulya Dairy Whitener",
        Quantity : "1 Kg pouch",
        price : "415",
        strikedOdprice : "425"

    },
    {
        image_adress:"https://www.bigbasket.com/media/uploads/p/s/1217239_1-nestle-everyday-dairy-whitener-milk-powder-for-tea.jpg",
        brand : "Nestle",
        name : "Everyday Dairy Whitener - Milk Powder for Tea",
        Quantity : "3x1 Kg Multipack",
        price : "1605",
        strikedOdprice : "1650"

    },
    {
        image_adress:"https://www.bigbasket.com/media/uploads/p/s/40009983_18-britannia-gobbles-fruity-fun-cake.jpg",
        brand : "Britannia",
        name : "Gobbles Bar Cake - Fruity Fun, Soft & Fluffy Snack",
        Quantity : "110 g",
        price : "27.30",
        strikedOdprice : "30"

    },
    {
        image_adress:"https://www.bigbasket.com/media/uploads/p/s/40005030_2-amul-fresh-cream-25-milk-fat-low-fat.jpg",
        brand : "Amul",
        name : "Fresh Cream - 25% Milk Fat Low Fat",
        Quantity : "1 L Carton",
        price : "205",
        strikedOdprice : "214"

    },
    {
        image_adress:"https://www.bigbasket.com/media/uploads/p/s/40088412_6-hersheys-milk-shake-chocolate.jpg",
        brand : "Hersheys",
        name : "Milk Shake - Chocolate",
        Quantity : "180 ml",
        price : "31.50",
        strikedOdprice : "35"

    },
]



displayProducts(dairyArr);

function displayProducts(data){
    data.map(function(elem,index){
        var div1 = document.createElement("div");
        var div2 = document.createElement("div");
        var div3 = document.createElement("div");
        div3.setAttribute("class","image-div");
        var image = document.createElement("img");
        image.setAttribute("src",elem.image_adress);
        div3.append(image);
        var div4 = document.createElement("div");
        var brand = document.createElement("h6");
        brand.innerText=elem.brand;
        var a1 = document.createElement("a");
        a1.innerText=elem.name;
        div4.append(brand,a1);
        var div5 = document.createElement("div");
        var size = document.createElement("h5");
        size.innerText=elem.Quantity;
        var span2 = document.createElement("span");
        var price = document.createElement("span");
        price.innerText="Rs "+elem.price; 
        price.style.fontWeight="bold"
        var strikedPrice = document.createElement("span");
        strikedPrice.innerText="Rs "+elem.strikedOdprice;
        strikedPrice.style.textDecoration="line-through"
        strikedPrice.style.fontSize="11px"
        span2.append(strikedPrice,price);
        var deleveryType = document.createElement("p");
        deleveryType.innerHTML='<i class="fa-solid fa-truck"></i> Standard Delevery Tomorrow Morning';
        var span4 = document.createElement("span");
        var span5= document.createElement("span");
        span5.innerText="Qty : ";
        var quantity = document.createElement("input");
        quantity.setAttribute("type","number");
        quantity.style.width="60px"
        var button = document.createElement("button");
       
       
        button.innerHTML='<i class="fa-solid fa-cart-plus"></i>';
        button.addEventListener("click",function(){
            addToCart(index);
        })
        span4.append(span5,quantity,button);
        div5.style.backgroundColor="rgb(226, 226, 226)"
        div5.style.padding="5px"
        div4.style.padding="5px"
        
        div4.style.fontWeight="bold"
        
        div5.append(size,span2,deleveryType,span4);
        div1.append(div2,div3,div4,div5)
        document.querySelector(".products-cards").append(div1);

    })
}

function sortPriceLowtoHigh(data){
    Arrays.sort(data)

}


var cartArr = JSON.parse(localStorage.getItem("cart")) || [];
function addToCart(index){
    
    var product = dairyArr.filter(function(elem,i){
        return index==i;
    })
    cartArr.push(product[0]);
    localStorage.setItem("cart",JSON.stringify(cartArr))

}